...
export default function WatchedFilter({
  hideWatched,
  onToggle,
  watchedCount = 0,
  totalCount = 0,
  ...
  return (
    <button
      onClick={() => onToggle(!hideWatched)}
      className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors font-medium text-sm ${
        hideWatched
          ? 'bg-blue-600 hover:bg-blue-700 text-white'
          : 'bg-neutral-700 hover:bg-neutral-600 text-gray-300'
      }`}
      title={hideWatched ? 'Show watched videos' : 'Hide watched videos'}
    >
      {hideWatched ? (
        <>
          ...
          <span className="hidden sm:inline">Show Watched</span>
        </>
      ) : (
        <>
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
